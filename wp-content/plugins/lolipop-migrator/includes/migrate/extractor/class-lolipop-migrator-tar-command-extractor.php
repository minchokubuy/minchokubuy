<?php
/**
 * Lolipop Migrator: Lolipop_Migrator_Tar_Command_Extractor class
 *
...
/**
 * Lolipop Migrator File Extract By tar command
 *
 * @since 0.1.0
 */
class Lolipop_Migrator_Tar_Command_Extractor extends Lolipop_Migrator_Base_Extractor {
	/**
	 * ファイルの展開を行う
	 *
	...
	public function extract( $archive_file ) {
		...
		$command = $command . ' xvfp ' . $archive_file
			. ' -C ' . dirname( WP_CONTENT_DIR ) . ' 2> ' . LOLIPOP_MIGRATOR_EXTRACT_ERROR_LOG_FILE
			. ' > /dev/null ';
		...
		if ( 0 !== $return_var || 0 < strlen( $content ) ) {
			$message = sprintf( '[error] exportor:%s - tar コマンドでの展開に失敗した - %s - %s', $class, $archive_file, $content );
			throw new Lolipop_Migrator_Extract_Exception( $message );
		}
	}
}
